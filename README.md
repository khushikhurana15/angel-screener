...
│                               # ETQ/avg price, market depth, live re-screening, ML prediction, explanations
...
- **Rate limiting**: Angel One's historical data and quote APIs enforce strict rate limits. The pipeline caches candle history per stock and only fetches incremental updates after the first load, with retry logic and backoff delays as an additional safeguard.
...
- Model retraining pipeline that runs on a schedule as new trade outcomes accumulate, with live-logged predictions compared against realized outcomes to track true out-of-sample accuracy over time